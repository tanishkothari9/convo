...
function buildScene(w: number, h: number): Scene {
  ...
  for (let i = 0; i < cloudCount; i += 1) {
    ...
    cc.fillStyle = "#000";
    cc.fillRect(
      Math.round(cx - scale * 2.6),
      Math.round(base),
      Math.round(scale * 5.2),
      clouds.height,
    );
    cc.globalCompositeOperation = "source-over";
    cc.fillStyle = P.cloudLow;
    cc.fillRect(
      Math.round(cx - scale * 1.4),
      Math.round(base) - 1,
      Math.round(scale * 2.8),
      1,
    );
    for (let j = 0; j < lobes; j += 1) {
      const l = lobeAt(j);
...
function paintStatic(
  ...
  const band = (ramp: readonly string[], y0: number, y1: number) => {
    const stops = ramp.map((_, i) =>
      Math.round(y0 + ((y1 - y0) * i) / (ramp.length - 1)),
    );
  ...
  for (let i = 0; i < Math.round(w * 1.6); i += 1) {
    ...
    c.fillStyle =
      gr() > 0.5 ? (depth > 0.5 ? P.grassSun : P.grassLit) : P.grassDark;
  ...
  for (let sx = -Math.round(sMin / 2); sx < w + 8;) {
    ...
    c.fillRect(
      ...
      bodyTop,
      sw - 2,
      Math.max(1, Math.round((stallBase - bodyTop) * 0.55)),
    );
  ...
  for (const p of puffs)
    roughBlob(cy, p.x, p.y - p.r * 0.16, p.r * 0.94, P.leaf);
  for (const p of puffs)
    roughBlob(cy, p.x - p.r * 0.16, p.y - p.r * 0.3, p.r * 0.72, P.leafMid);
  for (const p of puffs) {
    if (cn() > 0.72) continue;
    roughBlob(cy, p.x - p.r * 0.26, p.y - p.r * 0.44, p.r * 0.46, P.leafLit);
    if (cn() > 0.45)
      roughBlob(cy, p.x - p.r * 0.34, p.y - p.r * 0.56, p.r * 0.24, P.leafSun);
  }
  ...
  useEffect(() => {
    ...
    function draw(t: number) {
      ...
      for (const shaft of scene.shafts) {
        ctx.globalAlpha =
          0.025 + (Math.sin(t * 0.0004 + shaft.phase) * 0.5 + 0.5) * 0.035;
      ...
      for (const glint of scene.glints) {
        ctx.globalAlpha =
          0.55 + (Math.sin(t * 0.0014 + glint.phase) * 0.5 + 0.5) * 0.45;
