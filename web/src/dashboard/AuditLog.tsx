      ...
      {entries.length === 0 ? (
        <div className="empty">
          ...
          <p className="empty-body">
            The first cart lock, order, or payment attempt on the marketplace
            will appear here with its amount, its outcome, and the reason the
            agent gave.
          </p>
        </div>
      ) : (
        <>
          ...
          {visible.length === 0 ? (
            <p className="t-secondary" style={{ padding: "var(--space-6) 0" }}>
              Nothing{" "}
              {FILTERS.find((f) => f.key === filter)?.label.toLowerCase()} yet.
            </p>
          ) : (
                  ...
                  {visible.map((entry) => (
                    <Fragment key={entry.id}>
                      ...
                      >
                        <td className="cell-strong">
                          <span className="audit-action">
                            ...
                            >
                              {(() => {
                                const Icon =
                                  ACTION_ICONS[entry.actionType] ?? IconAudit;
                                return <Icon size={14} />;
                              })()}
                            </span>
                        ...
                        <td>
                          <span
                            className={`badge badge-dot ${outcomeClass(entry.outcome)}`}
                          >
                            {entry.outcome}
                          </span>
                        </td>
                        ...
                        <td className="t-id">
                          {entry.orderId ?? entry.cartId ?? "—"}
                        </td>
                        <td className="cell-right t-muted">
