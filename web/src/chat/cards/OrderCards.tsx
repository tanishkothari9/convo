...
export function CheckoutCard({
  ...
  const [states, setStates] = useState<Record<string, OrderState>>(() =>
    Object.fromEntries(
      orders.map((order) => [order.order_id, "open" as OrderState]),
    ),
  );
  ...
  const showForm =
    needsAddress &&
    (editingAddress || (address === null && saved.length === 0));
  ...
  const remember = (next: ShippingAddress) => {
    ...
    setSaved((current) => [
      next,
      ...current.filter((a) => addressKey(a) !== addressKey(next)),
    ]);
        ...
        for (const order of state.orders) {
          if (order.status === "paid") next[order.order_id] = "paid";
            ...
            if (order.failure_reason)
              why[order.order_id] = order.failure_reason;
          } else if (order.status === "failed") {
            next[order.order_id] = "failed";
            why[order.order_id] =
              order.failure_reason ?? "The payment did not go through.";
          } else next[order.order_id] = "open";
        }
        // Every order replaced at once means a newer checkout took over.
        const allCancelled =
          state.orders.length > 0 &&
          state.orders.every((o) => o.status === "cancelled");
        if (allCancelled) {
          setSuperseded(
  ...
  const next = orders.find(
    (order) =>
      states[order.order_id] === "open" || states[order.order_id] === "failed",
  );
  const settled = paid === orders.length;

  async function cancel(declineReason?: string) {
    const target = paying;
    setPaying(null);
    if (!target) return;
    ...
    try {
      ...
      const response = await api.post<{ checkout: CheckoutState }>(
        ...
        // A decline and a dismissal end up at the same endpoint; only this
        // tells them apart, in the ledger as well as on the card.
      ...
      setStates((current) => {
        ...
        for (const order of response.checkout.orders) {
          if (order.status === "cancelled")
            updated[order.order_id] = "cancelled";
        }
        return updated;
      });
    } catch {
    ...
    onSettled({
      paid: false,
      reason: declineReason ? "declined" : "cancelled",
    });
  }
    ...
    try {
      ...
      if (response.status === "paid") {
        ...
        setReasons((current) => ({
          ...current,
          [target.order_id]:
            response.failureReason ?? "The payment did not go through.",
        }));
        onSettled({
          paid: false,
    ...
    return (
      <div className="order-card">
        <div className="order-head">
          <p className="order-label t-sm t-muted">
            {split ? "Your orders" : "Your order"}
          </p>
    ...
    <div className="order-card">
      ...
      {split && (
        <p className="order-split-note t-sm">
          {orders.length} brands, so {orders.length} separate orders — each paid
          to that brand directly.
        </p>
      )}
          ...
          >
            {split && (
              <header className="order-brand-head">
                ...
                <span className="order-brand-total t-num">
                  {order.total_display}
                </span>
              </header>
            )}
      ...
      {payload.note && (
        <p className="order-note t-sm t-secondary">{payload.note}</p>
      )}
      ...
      {!settled &&
        ...
        address !== null && (
          <AddressSummary
            ...
            editable={!disabled}
            {...(split
              ? { note: "Everything in this checkout goes here." }
              : {})}
          />
        )}
      ...
      {!checking && settled && (
        <p className="notice notice-ok">
          {split
            ? "All paid. Your confirmations are below."
            : "Paid. Your confirmation is below."}
        </p>
      )}
      ...
      {paying && (
        <PaymentPanel
          ...
          customer={
            address ? { name: address.name, phone: address.phone } : null
          }
          onCancel={cancel}
          onResult={confirm}
        />
    ...
    <div className="confirm-card">
      <div className="confirm-head">
        ...
        <div className="confirm-body">
          ...
          <p className="confirm-title">
            {payload.brand_name ? `${payload.brand_name} — ` : ""}Paid{" "}
      ...
      {split && (
        <p className="confirm-remaining t-sm">
          One of {payload.orders_in_checkout} orders in this checkout, each paid
          to its own brand.
        </p>
      )}
