...
import { presentCart, presentProducts, presentSuggestions } from './presentation.js'
...
function splitStatus(input: Record<string, unknown>): {
  ...
  const status = typeof raw === 'string' ? sanitizeLabel(raw, STATUS_MAX_CHARS) : null
...
async function dispatch(
  ...
  switch (name) {
    ...
    case 'search_catalog': {
      ...
      const limit = clamp(Number(input.limit ?? config.maxSearchResults), 1, config.maxSearchResults)

      const results = await backend.searchProducts(
        session,
        query,
        {
          ...(typeof filters.category === 'string' ? { category: filters.category } : {}),
    ...
    case 'get_product_details': {
      ...
      if (!product) return ok(`No product with id ${productId} in this brand's catalogue.`)
    ...
    case 'manage_cart': {
      ...
      const quantity = Number.isFinite(Number(input.quantity)) ? Number(input.quantity) : 1

      if (action === 'view') return viewCart({ backend, session })
      if (productId === '') return failed(`manage_cart "${action}" needs a product_id.`)
      if (action === 'add') {
        return gatedAddToCart({ backend, config, session, productId, quantity })
      }
      if (action === 'update') {
        return gatedUpdateCartItem({ backend, config, session, productId, quantity })
      }
      if (action === 'remove') return gatedRemoveFromCart({ backend, session, productId })
        ...
        STOREFRONT_FENCE.fencePayload(
          {
            ...
            orders: history.map((order) => ({
              ...
              ...(order.failureReason ? { failure_reason: order.failureReason } : {}),
            })),
          },
          config.maxFencedChars,
        ),
