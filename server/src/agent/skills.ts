...
function parse(source: string, fallbackName: string): Skill {
  const match = /^---\n([\s\S]*?)\n---\n([\s\S]*)$/.exec(source)
  if (!match) return { name: fallbackName, description: '', body: source.trim() }
...
export function skills(): Skill[] {
  ...
  const files = readdirSync(SKILLS_DIR).filter((file) => file.endsWith('.md')).sort()
  cached = files.map((file) =>
    parse(readFileSync(join(SKILLS_DIR, file), 'utf8'), file.replace(/\.md$/, '')),
  )
  return cached
}
