...
function markerPattern(label: string): RegExp {
  return new RegExp(
    `<\\s*/?\\s*${label.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}(?![A-Za-z0-9_])(?:[^<>]*>)?`,
    "gi",
  );
}
...
export class Fence {
    ...
    if (maxChars !== undefined && out.length > maxChars) {
      ...
      out =
        maxChars > suffix.length
          ? out.slice(0, maxChars - suffix.length) + suffix
  ...
  wrap(body: string, maxChars: number = MAX_FENCED_CHARS): string {
    const sanitized = this.sanitizeText(body, maxChars).replace(
      LEADING_TURN_INDICATOR,
      "$1$2 -",
    );
    return `${this.open}\n${sanitized}\n${this.close}`;
  }
...
export function sanitizeLabel(text: string, maxChars = 60): string {
  return STOREFRONT_FENCE.sanitizeText(text, maxChars)
    .replace(/\s+/g, " ")
    .trim();
}
