...
/*
 * One cart write at a time per shopper.
 *
 * Keyed on the customer session rather than the conversation, because the cart
 * is. A turn's tool calls already overlap; now two of the shopper's own chats
 * can be open at once and writing to the same basket, and a lock per thread
 * would let them interleave on exactly the row it exists to protect.
 */
...
async function withCartLock<T>(
  ...
  const previous = cartLocks.get(customerSessionId) ?? Promise.resolve();
  ...
  try {
    ...
    if (cartLocks.get(customerSessionId) === next)
      cartLocks.delete(customerSessionId);
  }
}
 ...
 */
function reopenAfterCheckout(
  session: StorefrontSession,
  reasoning: string | null,
): void {
  const open = carts.ensureOpen(session.customerSessionId);
  if (open.items.length > 0) return;

  const locked = carts.latestLocked(session.customerSessionId);
  ...
  const onThisCart = orders
    .listForCustomer(session.customerSessionId, 50)
...
function cartComponent(
  cart: PricedCart,
  kind: "cart" | "cart_state" = "cart_state",
) {
...
export async function gatedAddToCart(args: {
  ...
  return withCartLock(session.customerSessionId, async () => {
    reopenAfterCheckout(
      session,
      "customer added another item after staging a checkout",
    );
    ...
    if (room <= 0) {
      return failed(
        `That item is already at the per-item limit of ${config.maxQuantityPerItem}.`,
      );
    }
    ...
    try {
      ...
      const capped =
        allowed < requested
          ? ` (capped at the per-item limit of ${config.maxQuantityPerItem})`
          : "";
      return ok(
        `Added ${productId} x${allowed}${capped}. Cart now has ${cartSummary(cart)}.`,
        [cartComponent(cart)],
      );
    } catch (error) {
...
export async function gatedUpdateCartItem(args: {
  ...
  return withCartLock(session.customerSessionId, async () => {
    reopenAfterCheckout(
      session,
      "customer changed a quantity after staging a checkout",
    );
    ...
    try {
      ...
      const capped =
        applied < requested
          ...
          : "";
      return ok(
        `Updated quantity${capped}. Cart now has ${cartSummary(cart)}.`,
        [cartComponent(cart)],
      );
    } catch (error) {
      if (error instanceof Unavailable) return held(STOCK_GATE, error.message);
...
export async function gatedRemoveFromCart(args: {
  ...
  return withCartLock(session.customerSessionId, async () => {
    reopenAfterCheckout(
      session,
      "customer removed an item after staging a checkout",
    );
    const blocked = provenanceOrCart(session, productId);
    if (blocked) return blocked;
    const cart = await backend.removeFromCart(session, productId);
    return ok(`Removed ${productId}. Cart now has ${cartSummary(cart)}.`, [
      cartComponent(cart),
    ]);
  });
}
...
export async function viewCart(args: {
  ...
  if (cart.lines.length === 0)
    return ok("The cart is empty.", [cartComponent(cart)]);
...
export async function gatedCheckout(args: {
  ...
  return withCartLock(session.customerSessionId, async () => {
    ...
    if (shortfalls.length > 0) {
      for (const brand of brandsOf(shortfalls)) {
        audit.record({
          ...
          amountMinor: sumOf(
            shortfalls.filter((line) => line.tenantId === brand),
          ),
    ...
    if (totalMinor <= 0 || totalMinor > config.maxOrderTotalMinor) {
      ...
      for (const brand of brandsOf(priced.lines)) {
        audit.record({
          ...
          actionType: "checkout.blocked",
          amountMinor: sumOf(
            priced.lines.filter((line) => line.tenantId === brand),
          ),
          currency: priced.currency,
          outcome: "blocked",
          reasoning: args.reasoning ?? null,
    ...
    const superseded = orders.pendingForCustomer(session.customerSessionId);
    ...
    const carriedAddress = needsShipping
    ...
    type Handle = Awaited<
      ReturnType<
        (typeof groups)[number]["provider"]["adapter"]["createPaymentOrder"]
      >
    >;
    const handles: {
      order: Order;
    ...
    for (const [index, group] of groups.entries()) {
      ...
      try {
        const handle = await group.provider.adapter.createPaymentOrder(
        ...
        if (handle.amountMinor !== group.amountMinor) {
          throw new ProviderApiError(
            "The payment provider acknowledged a different amount.",
          );
        }
        ...
        transaction(() => {
          ...
          for (const stagedOrder of staged) {
            ...
            audit.record({
              ...
              reasoning: args.reasoning ?? null,
              detail: {
                provider: stagedOrder.providerType,
                reason,
                checkout_id: checkoutId,
              },
            });
    ...
    transaction(() => {
      for (const { order, group, handle } of handles) {
        ...
        orders.setProviderOrderId(
          group.tenantId,
          order.id,
          handle.providerOrderId,
        );
        audit.record({
          tenantId: group.tenantId,
          conversationId: session.conversationId,
      ...
      [
        {
          ...
          payload: {
            ...
            saved_addresses: needsShipping
              ? orders.savedShippingAddresses(session.customerSessionId)
              : [],
            orders: handles.map(({ order, group, handle }) => ({
              ...
              lines: group.lineItems.map((line) => ({
                ...
                name: line.name,
                quantity: line.quantity,
                unit_price_display: formatMoney(
                  line.unitPriceMinor,
                  priced.currency,
                ),
                line_total_display: formatMoney(
                  line.lineTotalMinor,
                  priced.currency,
                ),
              })),
...
export async function gatedConfirmPayment(args: {
  ...
  if (order.status === "paid") {
    return ok(`Order ${order.id} is already paid.`, [
      orderConfirmationComponent(order.id, order),
    ]);
  }
      ...
      [
        {
          ...
          payload: {
            ...
            reason:
              order.failureReason ??
              "This order was replaced by a newer one and cannot be paid.",
  ...
  const { adapter, credentials, providerType } = resolveProvider(
    order.tenantId,
  );
  ...
  try {
    ...
    result = {
      ...
      failureReason:
        "Convo could not reach the payment provider to confirm this payment.",
    };
  }
