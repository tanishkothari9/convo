...
function withStatus(tool: ToolDefinition): ToolDefinition {
  ...
  return {
    ...
    parameters: {
      ...parameters,
      properties: { [STATUS_FIELD]: statusProperty, ...(parameters.properties ?? {}) },
    },
  }
}
  ...
  const readAndWrite: ToolDefinition[] = [
    {
      ...
      parameters: {
        type: 'object',
        properties: {
          skill: { type: 'string', enum: skillNames(), description: 'The skill to load.' },
        },
        required: ['skill'],
    ...
    },
    {
      ...
      parameters: {
        ...
        properties: {
          query: {
            type: 'string',
            description: 'What to search for. An empty query lists the catalogue.',
          },
          filters: {
            type: 'object',
            description: 'Constraints the customer stated; leave guesses in the query.',
            properties: {
              category: { type: 'string', description: 'Catalogue category name.' },
              min_price: { type: 'number', description: 'Lowest acceptable price, in whole currency units.' },
              max_price: { type: 'number', description: 'Price ceiling the customer stated, in whole currency units.' },
              sort: {
                type: 'string',
    ...
    },
    {
      ...
      parameters: {
        type: 'object',
        properties: {
          product_id: { type: 'string', description: 'A product_id a tool returned this conversation.' },
        },
        required: ['product_id'],
        additionalProperties: false,
      },
      ...
      parameters: {
        ...
        properties: {
          action: {
            ...
            description: 'What to do. `view` reads the cart without changing it.',
          },
          product_id: {
            type: 'string',
    ...
    },
    {
      ...
      parameters: {
        type: 'object',
        properties: {
          limit: { type: 'integer', minimum: 1, maximum: 10, description: 'How many orders, at most 10.' },
        },
        additionalProperties: false,
      },
    },
  ...
  const presentation: ToolDefinition[] = [
    {
      ...
      parameters: {
        type: 'object',
        properties: {
          title: { type: 'string', maxLength: 80, description: 'Short heading for the set of cards.' },
          layout: {
            type: 'string',
          ...
          picks: {
            ...
            items: {
              type: 'object',
              properties: {
                product_id: { type: 'string', description: 'product_id returned by a tool this conversation.' },
                reason: {
                  type: 'string',
                  maxLength: 140,
                  description: 'One clause tying the pick to a need the customer stated.',
                },
              },
              required: ['product_id'],
              additionalProperties: false,
      ...
      parameters: {
        ...
        properties: {
          suggestions: {
            ...
            minItems: 1,
            maxItems: 4,
            description: '1-4 chips, each a brief imperative and each a different kind of step.',
          },
        },
...
export const PRESENTATION_TOOLS = new Set(['present_products', 'present_cart', 'present_suggestions'])
