  ...
  switch (intent.kind) {
          ...
          call("search_catalog", {
            ...
            intent.sort
              ? {
                  filters: {
                    ...(intent.maxPriceMinor !== undefined
                      ? { max_price: intent.maxPriceMinor / 100 }
                      : {}),
                    ...(intent.minPriceMinor !== undefined
                      ? { min_price: intent.minPriceMinor / 100 }
                      : {}),
    ...
    case "view_cart":
      return {
        text: "",
        toolCalls: [
          call("manage_cart", { status: "opening your cart", action: "view" }),
        ],
        stopReason: "tool_use",
      };
...
function present(
  ...
  if (failed) {
    return {
      ...
      toolCalls: has("present_suggestions")
        ? [
            call("present_suggestions", {
              suggestions: ["Show me something else", "Open my cart"],
            }),
          ]
        : [],
      stopReason: "end_turn",
  ...
  if (heldResult) {
    return {
      ...
      toolCalls: has("present_suggestions")
        ? [
            call("present_suggestions", {
              suggestions: heldChips(heldResult.content),
            }),
          ]
        : [],
      stopReason: "end_turn",
  ...
  if (cart) {
    ...
    return {
      ...
      text: empty
        ...
        : summarise(cart.content, wasWrite),
      toolCalls: [
  ...
  return {
    ...
    toolCalls: has("present_suggestions")
      ? [
          call("present_suggestions", {
            suggestions: ["Show me more", "Open my cart"],
          }),
        ]
      : [],
    stopReason: "end_turn",
...
function searchCount(messages: ModelMessage[]): number {
  ...
  for (const message of messages) {
    ...
    count += message.toolCalls.filter(
      (call) => call.name === "search_catalog",
    ).length;
  }
  return count;
}
...
function readIntent(messages: ModelMessage[]): Intent {
  ...
  if (
    /^(hi|hey|hello|namaste|yo|hola|good (morning|afternoon|evening))\b/.test(
      text,
    )
  ) {
  ...
  if (
    /\b(what can you do|help me|how does this work|who are you)\b/.test(text)
  ) {
    return { kind: "help" };
  }
  if (
    /\b(check ?out|place (the |my )?order|pay now|proceed to pay|buy it now|complete (my )?order)\b/.test(
      text,
    )
  ) {
  ...
  if (
    /\b(add|buy|i'?ll take|take the|get me|i want the|put .* in (my |the )?cart|order the)\b/.test(
      text,
    )
  ) {
    return {
  ...
  if (
    /\b(my cart|the cart|show cart|open cart|what'?s in (my |the )?cart|view cart|basket)\b/.test(
      text,
    )
  ) {
...
function readSort(text: string): { sort?: "price_asc" | "price_desc" } {
  if (
    /\b(cheap|cheapest|cheaper|budget|affordable|lowest|least expensive|inexpensive)\b/.test(
      text,
    )
  ) {
  ...
  if (
    /\b(expensive|premium|finest|luxury|dearest|priciest|most expensive|high end)\b/.test(
      text,
    )
  ) {
...
function readPriceBounds(text: string): {
  maxPriceMinor?: number;
  minPriceMinor?: number;
} {
  ...
  const under = text.match(
    /\b(?:under|below|less than|cheaper than|upto|up to|within|max(?:imum)?)\s*₹?\s*([\d,]+)\s*(k|thousand)?/,
  );
  const over = text.match(
    /\b(?:over|above|more than|at least|min(?:imum)?)\s*₹?\s*([\d,]+)\s*(k|thousand)?/,
  );
  const bounds: { maxPriceMinor?: number; minPriceMinor?: number } = {};
  if (under?.[1]) {
    const base = normalise(under[1]);
    if (base !== undefined)
      bounds.maxPriceMinor = under[2] ? base * 1000 : base;
  }
  if (over?.[1]) {
...
function heldChips(content: string): string[] {
  if (content.includes("[held: empty_cart]"))
    return ["Show me what you have", "What is popular"];
  if (content.includes("[held: stock]"))
    return ["Remove it and check out", "Find something similar"];
  ...
  if (name === "search_catalog") {
    return call("search_catalog", {
      ...
      query:
        intent.kind === "search"
          ? intent.query
          : cleanQuery(lastUserText(messages)),
      limit: 6,
    });
  }
  if (name === "manage_cart")
    return call("manage_cart", { status: "opening your cart", action: "view" });
  return call(name, {});
}
