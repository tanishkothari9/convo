...
export class AnthropicModelProvider implements ModelProvider {
  ...
  async *streamAgentTurn(
    ...
    const open = new Map<number, { id: string; name: string; json: string; statusSent: boolean }>()
      ...
      switch (event.type) {
        case 'content_block_start': {
          ...
          if (block?.type === 'tool_use') {
            ...
            yield { type: 'tool_call_start', id: String(block.id), name: String(block.name) }
          }
          break
        }
        ...
        case 'content_block_stop': {
          ...
          if (pending) {
            open.delete(index)
            const call = { id: pending.id, name: pending.name, input: parseToolInput(pending.json) }
            toolCalls.push(call)
        ...
        case 'error': {
          ...
          throw new ModelProviderError(err?.message ?? 'The Anthropic API reported an error.', true)
        }
        default:
          break
      }
