...
export function errorHandler(err: unknown, _req: Request, res: Response, _next: NextFunction) {
  ...
  log.error('unhandled request error', { message: err instanceof Error ? err.message : String(err) })
...
export function requireString(body: unknown, field: string, max = 500): string {
  ...
  if (value.length > max) throw badRequest(`\`${field}\` is too long.`, 'field_too_long')
  return value.trim()
}

export function optionalString(body: unknown, field: string, max = 2000): string | undefined {
  const value = (body as Record<string, unknown> | null)?.[field]
...
export function requireInt(body: unknown, field: string, min = 0, max = 1_000_000_000): number {
