...
import { LiveRazorpayApi, type RazorpayApi, type RazorpayItem } from './client.js'
...
export class RazorpayAdapter implements CommerceProviderAdapter {
  ...
  private api(credentials: RazorpayCredentials): { api: RazorpayApi; keyId: string; keySecret: string } {
    ...
    return { api: mockRazorpay, keyId: MOCK_KEY_ID, keySecret: MOCK_KEY_SECRET }
  }

  async verifyCredentials(credentials: RazorpayCredentials): Promise<{ ok: true; detail: string }> {
    const { api } = this.api(credentials)
    if (api.isMock) {
      return {
        ok: true,
        detail: 'Connected to the Razorpay test sandbox built into Convo. Add test keys to use your own account.',
      }
    }
  ...
  async verifyPayment(
    ...
    const expectedOrderId = asString(payload.expectedOrderId) ?? reportedOrderId
    ...
    const expected = hmacSha256Hex(keySecret, `${expectedOrderId}|${paymentId}`)
    ...
    try {
      ...
      if (payment.status !== 'captured' && payment.status !== 'authorized') {
        return refuse(payment.error_description ?? 'The payment did not complete.')
      }
      return {
        verified: true,
