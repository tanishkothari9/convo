...
export class MockRazorpayApi implements RazorpayApi {
  ...
  async fetchItems(params: { count?: number; skip?: number; active?: 1 | 0 } = {}) {
    const skip = params.skip ?? 0
    const count = Math.min(params.count ?? 100, 100)
    const filtered =
      params.active === undefined ? MOCK_ITEMS : MOCK_ITEMS.filter((i) => i.active === (params.active === 1))
    const items = filtered.slice(skip, skip + count)
    return { entity: 'collection', count: items.length, items } satisfies RazorpayItemCollection
  }
  ...
  async createOrder(body: {
    ...
    if (!Number.isInteger(body.amount) || body.amount < 100) {
      ...
      throw new ProviderApiError('Order amount must be at least ₹1.', 400, 'BAD_REQUEST_ERROR')
    }
  ...
  async fetchOrder(orderId: string): Promise<RazorpayOrder> {
    ...
    if (!order) throw new ProviderApiError('The id provided does not exist', 400, 'BAD_REQUEST_ERROR')
    return order
  }

  async fetchPayment(paymentId: string): Promise<RazorpayPayment> {
    const payment = payments.get(paymentId)
    if (!payment) throw new ProviderApiError('The id provided does not exist', 400, 'BAD_REQUEST_ERROR')
    return payment
  }
