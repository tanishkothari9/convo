...
export class ShopifyAdapter implements CommerceProviderAdapter {
  ...
  private async request<T>(credentials: ShopifyCredentials, path: string): Promise<T> {
    const shop = normaliseShopDomain(credentials.shop ?? '')
    const token = credentials.accessToken?.trim()
    if (!token) throw new ProviderConfigError('Shopify needs an Admin API access token.')

    let response: Response
    try {
      response = await fetch(`https://${shop}/admin/api/${API_VERSION}${path}`, {
        headers: {
    ...
    if (!response.ok) {
      throw new ProviderApiError(`Shopify returned ${response.status}.`, response.status)
    }
    return (await response.json()) as T
  }

  async verifyCredentials(credentials: ShopifyCredentials): Promise<{ ok: true; detail: string }> {
    const shop = normaliseShopDomain(credentials.shop ?? '')
      ...
      .map((product) => {
        ...
        return {
          ...
          images: product.images
            ...
            .map((image) => image.src)
            .filter((src) => /^https:\/\//i.test(src) && !/\.svgz?($|[?#])/i.test(src))
            .slice(0, 6),
  ...
  async verifyPayment(): Promise<PaymentResult> {
    throw new ProviderConfigError('Shopify does not process payments for Convo.')
  }
}
