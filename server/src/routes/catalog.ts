...
function listingReadiness(tenant: Tenant) {
  ...
  if (products.list(tenant.id).length === 0) {
    blockers.push(
      "Add at least one product before listing on the marketplace.",
    );
  }
  if (!connections.activePayment(tenant.id)) {
    blockers.push(
      "Connect a payment provider before listing on the marketplace.",
    );
  }
...
catalogRoutes.patch(
  ...
  route(async (req, res) => {
    ...
    if (rawSlug !== undefined) {
      ...
      if (slug.length < 2)
        throw badRequest("A link needs at least two characters.", "bad_slug");
      if (tenants.slugTaken(slug, tenant.id))
        throw conflict("That link is taken.", "slug_taken");
      patch.slug = slug;
    }
    ...
    if (typeof req.body?.isListed === "boolean") {
      if (req.body.isListed) {
        ...
        if (blockers.length > 0)
          throw badRequest(blockers[0]!, "not_ready_to_list");
      }
...
catalogRoutes.get(
  ...
  route(async (req, res) => {
    res.json({
      products: products.list(tenantOf(req).id, { includeInactive: true }),
    });
  }),
);
  ...
  route(async (req, res) => {
    ...
    res.status(201).json({
      product: products.create({
        tenantId: tenant.id,
        ...readProduct(req.body, tenant.currency),
      }),
    });
  }),
);
...
catalogRoutes.patch(
  ...
  route(async (req, res) => {
    ...
    if (req.body?.priceMajor !== undefined) {
      patch.priceMinor = toMinor(
        requireInt(req.body, "priceMajor", 0, 10_000_000),
      );
    }
    if (req.body?.stock !== undefined)
      patch.stock = requireInt(req.body, "stock", 0, 1_000_000);
    ...
    if (req.body?.attributes !== undefined)
      patch.attributes = readAttributes(req.body.attributes);
...
catalogRoutes.delete(
  "/products/:productId",
  route(async (req, res) => {
    if (!products.remove(tenantOf(req).id, req.params.productId!))
      throw notFound("No such product.");
    res.status(204).end();
  }),
);
...
catalogRoutes.post(
  ...
  route(async (req, res) => {
    ...
    if (live.length >= 10) {
      throw badRequest(
        "You already have 10 active keys. Revoke one first.",
        "too_many_keys",
      );
    }
...
export function readImages(value: unknown): string[] {
  ...
  return value
    .filter(
      (url): url is string => typeof url === "string" && url.length < 4000,
    )
    .map((url) => url.trim())
...
export function isSafeImageUrl(url: string): boolean {
  ...
  if (/^http:\/\//i.test(url))
    return !env.isProduction && !/\.svgz?($|[?#])/i.test(url);
  return false;
}
...
function readAttributes(value: unknown): Record<string, string> {
  ...
  for (const [key, raw] of Object.entries(
    ...
    if (typeof raw === "string" && key.length <= 40 && raw.length <= 120)
      out[key] = raw;
  }
  return out;
}
