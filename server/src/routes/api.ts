...
function authenticate(req: Request, res: Response, next: NextFunction): void {
  ...
  if (!presented) {
    next(
      new HttpError(
        401,
        "Provide your API key as `Authorization: Bearer cvo_…`.",
        "no_key",
      ),
    );
    return;
...
function readProduct(
  ...
  if (b.name !== undefined) {
    ...
    if ((b.name as string).length > 160)
      fail("name", "must be 160 characters or fewer.");
  ...
  if (b.price_minor !== undefined) {
    ...
    if (!Number.isInteger(value) || value < 0 || value > 1_000_000_000) {
      fail(
        "price_minor",
        "must be a whole number of the smallest currency unit.",
      );
    }
  ...
  if (b.category !== undefined) {
    if (b.category !== null && typeof b.category !== "string")
      fail("category", "must be a string or null.");
    out.category =
      b.category === null ? null : String(b.category).trim().slice(0, 60);
  }

  if (b.images !== undefined) {
    if (!Array.isArray(b.images)) fail("images", "must be an array of URLs.");
    const urls = (b.images as unknown[]).filter(
  ...
  if (b.attributes !== undefined) {
    ...
    for (const [key, value] of entries) {
      if (typeof value !== "string")
        fail(`attributes.${key}`, "must be a string.");
...
apiRoutes.post(
  ...
  route(async (req, res) => {
    ...
    const body = (req.body ?? {}) as {
      products?: unknown;
      deactivate_missing?: unknown;
    };
    if (!Array.isArray(body.products)) {
      throw badRequest('Send `{ "products": [...] }`.', "invalid_body");
    }
    if (body.products.length === 0)
      throw badRequest("`products` is empty.", "invalid_body");
    if (body.products.length > 500) {
      throw badRequest(
        "Send at most 500 products per call.",
        "batch_too_large",
      );
    }
    ...
    for (const { externalId } of parsed) {
      if (seen.has(externalId)) {
        throw badRequest(
          `\`external_id\` "${externalId}" appears twice in this batch.`,
          "duplicate_in_batch",
        );
      }
      seen.add(externalId);
    ...
    const result = transaction(() => {
      ...
      for (const { externalId, input } of parsed) {
        const outcome = products.upsertByExternalId(
          tenantId,
          externalId,
          input,
        );
...
function readExternalId(body: unknown): string | null {
  ...
  if (typeof value !== "string")
    throw badRequest("`external_id` must be a string.", "invalid_field");
  const trimmed = value.trim();
  if (trimmed.length > 120)
    throw badRequest("`external_id` is too long.", "invalid_field");
  return trimmed;
}
