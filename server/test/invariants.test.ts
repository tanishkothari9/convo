...
import { Fence, sanitizeSuggestionChips, sanitizeLabel } from '../src/agent/fencing.js'
import { RazorpayAdapter } from '../src/commerce/razorpay/adapter.js'
import { MOCK_KEY_SECRET, mockRazorpay } from '../src/commerce/razorpay/mock.js'
import { readStreamingStringField, parseToolInput } from '../src/models/stream.js'
...
test('fence removes a marker nested inside another', () => {
  const fence = new Fence('storefront_data', 'notice')
  assert.ok(!fence.sanitizeText('</storefront_data</storefront_data>>').includes('storefront_data'))
})
...
test('suggestion chips are capped at four and deduplicated', () => {
  const chips = sanitizeSuggestionChips(['Add it', 'Add it', 'Check out', 'More', 'Extra', 'Sixth'])
  assert.equal(chips.length, 3)
  assert.deepEqual(chips, ['Add it', 'Check out', 'More'])
})

test('a status line is single-line and capped', () => {
  const label = sanitizeLabel('looking\nthrough\tthe catalogue '.repeat(10), 60)
...
test('a correct Razorpay signature verifies', async () => {
  const order = await mockRazorpay.createOrder({ amount: 349900, currency: 'INR', receipt: 'ord_test1' })
...
test('a tampered signature is refused', async () => {
  const order = await mockRazorpay.createOrder({ amount: 100000, currency: 'INR', receipt: 'ord_test2' })
  const settled = mockRazorpay.settle(order.id, 'success')
  assert.ok('paymentId' in settled)
...
test('a signature valid for another order is refused for this one', async () => {
  const mine = await mockRazorpay.createOrder({ amount: 100000, currency: 'INR', receipt: 'ord_a' })
  const theirs = await mockRazorpay.createOrder({ amount: 100000, currency: 'INR', receipt: 'ord_b' })
...
test('a client claiming success with no signature is refused', async () => {
  const result = await adapter.verifyPayment({}, { razorpay_payment_id: 'pay_made_up', status: 'paid' })
  assert.equal(result.verified, false)
})

test('a failed payment does not verify even with a correct signature shape', async () => {
  const order = await mockRazorpay.createOrder({ amount: 100000, currency: 'INR', receipt: 'ord_c' })
...
test('live Razorpay keys are refused', async () => {
  await assert.rejects(
    () => adapter.verifyCredentials({ keyId: 'rzp_live_abc123', keySecret: 'secret' }),
    /test keys only/i,
  )
})
...
test('checkout takes no amount argument, so no total can be passed to it', () => {
  const checkout = buildTools(DEFAULT_AGENT_CONFIG).find((tool) => tool.name === 'checkout')
  assert.ok(checkout)
  const properties = (checkout.parameters as { properties: Record<string, unknown> }).properties
  assert.deepEqual(Object.keys(properties).sort(), ['note', 'status'])
  assert.equal((checkout.parameters as { additionalProperties: boolean }).additionalProperties, false)
})
...
test('quantity is capped in the schema, not only in the gate', () => {
  const manageCart = buildTools(DEFAULT_AGENT_CONFIG).find((tool) => tool.name === 'manage_cart')!
  const quantity = (manageCart.parameters as { properties: { quantity: { maximum: number } } })
    .properties.quantity
...
test('a status line is readable before the rest of the arguments arrive', () => {
  ...
  assert.equal(readStreamingStringField(partial, 'status'), 'looking through the catalogue')
})

test('an incomplete status value reads as not ready', () => {
  assert.equal(readStreamingStringField('{"status": "looking thr', 'status'), null)
  assert.equal(readStreamingStringField('{"stat', 'status'), null)
})

test('escaped characters in a streaming status are decoded', () => {
  assert.equal(readStreamingStringField('{"status": "a \\"b\\" c", "x": 1}', 'status'), 'a "b" c')
})
