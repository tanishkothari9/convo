...
const { mintApiKey, hashApiKey, digestsMatch, readBearer } =
  await import("../src/lib/apikeys.js");
const { isSafeImageUrl } = await import("../src/routes/catalog.js");
const { encryptJson, decryptJson, hashPassword, verifyPassword } =
...
before(() => {
  db();
  alpha = tenants.create({
    name: "Alpha Brand",
    slug: `alpha-${process.pid}`,
  }).id;
...
test("one brand cannot read another brand’s products", () => {
  ...
  assert.equal(
    products.byId(alpha, secret.id),
    undefined,
    "byId crossed tenants",
  );
  assert.deepEqual(
    products.byIds(alpha, [secret.id]),
    [],
    "byIds crossed tenants",
  );
  assert.ok(
    !products
      .list(alpha, { includeInactive: true })
      .some((p) => p.id === secret.id),
  );
});
...
test("one brand cannot update or delete another brand’s products", () => {
  ...
  assert.equal(
    products.update(alpha, secret.id, { name: "hijacked", priceMinor: 1 }),
    undefined,
  );
...
test("an external_id is unique per brand, not globally", () => {
  const a = products.upsertByExternalId(alpha, "SKU-SHARED", {
    name: "Alpha item",
    priceMinor: 100,
  });
  const b = products.upsertByExternalId(beta, "SKU-SHARED", {
    name: "Beta item",
    priceMinor: 200,
  });
...
test("orders and audit entries do not cross tenants", () => {
  const shopper = `cust-${process.pid}`;
  const conversation = conversations.ensure(shopper);
  const cart = carts.ensureOpen(shopper);
...
test("a shopper cannot read an order belonging to someone else", () => {
  const me = `shopper-a-${process.pid}`;
  const them = `shopper-b-${process.pid}`;
  const mine = conversations.ensure(me);
  conversations.ensure(them);
  const cart = carts.ensureOpen(me);
  const order = orders.create({
    tenantId: beta,
    cartId: cart.id,
  ...
  assert.ok(
    orders.forCustomer(me, order.id),
    "the owner cannot read their own order",
  );
  assert.equal(
    orders.forCustomer(them, order.id),
    undefined,
    "an order leaked across shoppers",
  );
  assert.deepEqual(
    orders.byCheckout(them, order.checkoutId),
    [],
    "a checkout leaked across shoppers",
  );
  assert.ok(!orders.listForCustomer(them, 50).some((o) => o.id === order.id));
  assert.equal(orders.lastShippingAddress(them), null);
});

/*
 * The other half of the same rule. Orders are reachable from every chat the
 * shopper owns — otherwise "where is my parcel" stops working the moment they
 * press New chat — and from none of anybody else's.
 */
test("an order placed in one chat is reachable from the shopper’s other chats", () => {
  const me = `shopper-multi-${process.pid}`;
  const first = conversations.ensure(me);
  const second = conversations.create(me);
  assert.notEqual(
    first.id,
    second.id,
    "a shopper must be able to hold two chats",
  );
  ...
  const order = orders.create({
    ...
    checkoutId: `cko-multi-${process.pid}`,
    totalAmountMinor: 9_900,
    currency: "INR",
    providerType: "manual",
    providerOrderId: "z",
    lineItems: [],
  ...
  // Placed in the first chat, found from the shopper — which is what the
  // second chat asks with.
  assert.ok(orders.forCustomer(me, order.id));
  assert.ok(orders.listForCustomer(me, 50).some((o) => o.id === order.id));
  assert.equal(orders.byCheckout(me, order.checkoutId).length, 1);

  // And the basket is the same basket in both.
  assert.equal(carts.ensureOpen(me).id, cart.id);
});

test("archiving a chat hides it from the list without touching its orders", () => {
  const me = `shopper-archive-${process.pid}`;
  const kept = conversations.ensure(me);
  const gone = conversations.create(me);
  const order = orders.create({
    tenantId: beta,
    cartId: carts.ensureOpen(me).id,
    conversationId: gone.id,
    checkoutId: `cko-arch-${process.pid}`,
    totalAmountMinor: 1_100,
    currency: "INR",
    providerType: "manual",
    providerOrderId: "a",
    lineItems: [],
  ...
  assert.equal(conversations.archive(me, gone.id), true);
  const listed = conversations.list(me).map((c) => c.id);
  assert.ok(listed.includes(kept.id));
  assert.ok(!listed.includes(gone.id), "an archived chat is still in the list");

  // The money survives the tidying.
  assert.ok(
    orders.forCustomer(me, order.id),
    "archiving a chat took its order with it",
  );
});

test("a shopper cannot archive or open a chat that is not theirs", () => {
  const me = `owner-${process.pid}`;
  const them = `intruder-${process.pid}`;
  const mine = conversations.ensure(me);
  assert.equal(
    conversations.owned(them, mine.id),
    undefined,
    "a chat leaked across shoppers",
  );
  assert.equal(
    conversations.archive(them, mine.id),
    false,
    "someone else archived my chat",
  );
  assert.ok(conversations.owned(me, mine.id));
});

test("a shopper’s cart spans brands but not other shoppers", () => {
  const me = `cart-a-${process.pid}`;
  const them = `cart-b-${process.pid}`;
  const alphaItem = products.create({
    tenantId: alpha,
    name: "Alpha good",
    priceMinor: 1000,
    stock: 5,
  });
  const betaItem = products.create({
    tenantId: beta,
    name: "Beta good",
    priceMinor: 2000,
    stock: 5,
  ...
  assert.notEqual(carts.ensureOpen(them).id, cart.id);
  assert.equal(carts.ensureOpen(them).items.length, 0);
});

test("a brand that has not opted in is not on the marketplace shelf", () => {
  const hidden = products.create({
    tenantId: alpha,
    name: "Unlisted good",
    priceMinor: 500,
    stock: 3,
  ...
  assert.equal(
    products.listedById(hidden.id),
    undefined,
    "an unlisted brand reached the shelf",
  );
  ...
  assert.ok(
    products.listedById(hidden.id),
    "an opted-in brand did not reach the shelf",
  );
  assert.ok(products.listedAcrossBrands().some((p) => p.id === hidden.id));
...
test("a shopper’s conversation is not reachable from another session id", () => {
  ...
  assert.equal(
    provenance.has(theirs.id, item.id),
    false,
    "provenance leaked across shoppers",
  );
});
...
test("an API key is not recoverable from what is stored", () => {
  ...
  assert.ok(
    !stored.includes(minted.secret),
    "the secret is retrievable from the key list",
  );
...
test("ordinary product imagery is still accepted", () => {
  assert.equal(
    isSafeImageUrl("https://images.unsplash.com/photo-123?w=800"),
    true,
  );
...
test("a Shopify store name can never become an arbitrary host", () => {
  ...
  for (const value of hostile) {
    ...
    assert.ok(
      !resolved.includes("evil.example"),
      `"${value}" reached an attacker host`,
    );
  }
});
...
test("one caller being throttled does not throttle another", () => {
  ...
  assert.equal(
    limiter.take("caller-b").allowed,
    true,
    "buckets leaked between callers",
  );
});
