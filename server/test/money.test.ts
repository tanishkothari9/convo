...
const { carts, connections, orders, products, provenance, tenants, audit } =
...
const {
  ...
  gatedConfirmPayment,
  viewCart,
} = await import("../src/agent/gates.js");
const { ConvoStorefront, ensureSession, priceCart } =
  await import("../src/agent/storefront.js");
...
async function alsoFrom(
  ...
  provenance.remember(session.conversationId, [
    { productId: product.id, tenantId: otherTenantId },
  ]);
  await gatedAddToCart({
    backend,
    config,
    session,
...
test("checkout is held on an empty cart and nothing is staged", async () => {
  ...
  assert.equal(orders.listForCustomer(session.customerSessionId).length, 0);
});
...
test("the charged total is recomputed from the catalogue, not from the cart snapshot", async () => {
  ...
  const order = orders.listForCustomer(session.customerSessionId)[0]!;
...
test("an item that sells out between the cart and checkout stops the charge", async () => {
  ...
  const blocked = audit
    ...
    .find(
      (entry) =>
        entry.actionType === "checkout.blocked" && entry.outcome === "blocked",
    );
...
test("a correctly signed payment marks the order paid and takes the stock", async () => {
  ...
  assert.equal(
    ...
    3,
    "stock comes off only once paid",
  );

  // The receipt has to say where it is going, or the customer never gets a
  // chance to notice it is headed to the wrong house.
  const receipt = outcome.components.find(
    (c) => c.component === "order_confirmation",
  )!;
  const shownAddress = receipt.payload.shipping_address as {
    city?: string;
  } | null;
  assert.equal(
    shownAddress?.city,
    "Mysuru",
    "the confirmation did not show the delivery address",
  );
  assert.ok(
    audit
      .list(tenantId, 50)
      .some(
        (e) => e.actionType === "payment.confirmed" && e.orderId === order.id,
      ),
  );
});
...
test("an order superseded by a newer checkout cannot be paid", async () => {
  ...
  await gatedCheckout({ session, config });
  const first = orders.listForCustomer(session.customerSessionId)[0]!;
...
test("shopping on after staging a checkout keeps the cart rather than starting a new one", async () => {
  ...
  await gatedCheckout({ session, config });
  const latest = orders.listForCustomer(session.customerSessionId)[0]!;
...
test("a cart write for a product this conversation has not seen is held", async () => {
  ...
  assert.equal(carts.ensureOpen(session.customerSessionId).items.length, 0);
});
...
test("an order with nowhere to send it cannot be paid, even with a valid signature", async () => {
  ...
  assert.equal(
    products.byId(tenantId, product.id)!.stock,
    5,
    "stock left the shelf without an address",
  );
});
...
test("the address is frozen onto the order it was given for", async () => {
  ...
  assert.equal(
    orders.byId(tenantId, order.id)!.shippingAddress!.city,
    "Bengaluru",
  );
});
...
test("an address is pre-filled from the last one used in the same conversation", async () => {
  ...
  const recalled = orders.lastShippingAddress(session.customerSessionId)!;
...
test("an address that could not be delivered to is refused", () => {
  ...
  for (const [input, field] of bad) {
    assert.throws(
      () => readAddress(input),
      (error: unknown) =>
        error instanceof AddressError && error.field === field,
...
test("a returning customer is not asked for the same address twice", async () => {
  ...
  const outcome = await gatedCheckout({ session, config });
  const second = orders.listForCustomer(session.customerSessionId)[0]!;

  assert.notEqual(second.id, first.id);
  assert.equal(
    second.shippingAddress?.postalCode,
    "570001",
    "the address was not carried forward",
  );

  const card = outcome.components.find((c) => c.component === "checkout")!;
  assert.ok(
    card.payload.shipping_address,
    "the card should open showing where this is going",
  );
...
test("the cart the page is handed is the cart the agent emits", async () => {
  ...
  const emitted = outcome.components!.find(
    (c) => c.component === "cart_state",
  )!.payload;
  const handed = cartPayload(
  ...
  assert.deepEqual(
    handed,
    emitted,
    "the route and the component disagree about the cart",
  );
...
test("a cart spanning two brands is charged as one order per brand", async () => {
  ...
  const staged = orders.listForCustomer(session.customerSessionId);
  assert.equal(
    staged.length,
    2,
    "a two-brand cart did not split into two orders",
  );
  assert.deepEqual(
  ...
  const byBrand = new Map(
    staged.map((order) => [order.tenantId, order.totalAmountMinor]),
  );
...
test("paying one brand in a split checkout does not settle the other", async () => {
  ...
  assert.equal(
    orders.byId(second.tenantId, second.id)!.status,
    "awaiting_payment",
  );
  ...
  assert.equal(
    carts.byId(first.cartId)!.status,
    "converted",
    "the cart did not close once both brands were paid",
  );
});
...
test("a half-paid checkout does not hand the cart back", async () => {
  ...
  assert.notEqual(
    carts.byId(first.cartId)!.status,
    "open",
    "a half-paid cart was handed back",
  );
});
...
test("a customer who has sent to several places gets a list, deduplicated", async () => {
  ...
  const send = async (address: Record<string, unknown>) => {
    ...
    await gatedConfirmPayment({
      ...
      payload: {
        payment_id: payment,
        signature: signManualPayment(order.providerOrderId!, payment),
      },
    });
  };
  ...
  const saved = orders.savedShippingAddresses(session.customerSessionId);
...
test("the same street with a different recipient is a separate entry", async () => {
  ...
  assert.equal(
    saved.length,
    2,
    "a gift to someone else at the same flat is a different delivery",
  );
});
...
test("the saved list is capped so a card cannot become a scrolling problem", async () => {
  ...
  assert.equal(
    orders.savedShippingAddresses(session.customerSessionId).length,
    5,
  );
});
